...
export default function App() {
  ...
  return (
    ...
    >
      ...
      {/* Tightened Main Container Layout */}
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-7 space-y-6 sm:space-y-8">
        {/* Compact Hero Section & SearchBar */}
        <section className="text-center max-w-3xl mx-auto space-y-3.5">
          <h1 className={`text-2xl sm:text-4xl font-extrabold tracking-tight leading-tight transition-colors ${
        ...
        {showAiRecommender && (
          <section className={`border rounded-2xl p-4 sm:p-6 space-y-4 backdrop-blur-xl shadow-xl transition-colors ${
            isDarkMode
            ...
            <div className={`flex items-center justify-between border-b pb-3 ${isDarkMode ? 'border-slate-800' : 'border-slate-200'}`}>
              ...
              <span className="text-[11px] text-indigo-500 bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded-full font-mono">
        ...
        {/* AI Reasoning Insights & Catalog Grid Section (Tightened Spacing) */}
        <section className="space-y-4 sm:space-y-5">
          {/* Error Banner */}
          {error && (
            <div className="bg-rose-950/40 border border-rose-800/60 rounded-2xl p-3.5 flex items-center justify-between text-rose-300 text-xs sm:text-sm">
              <div className="flex items-center gap-2">
          ...
          {isFiltered && reasoning && (
            <div className={`border rounded-2xl p-3.5 sm:p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-lg transition-colors ${
              isDarkMode
                ? 'bg-indigo-950/50 border-indigo-500/40 shadow-indigo-950/30'
                : 'bg-indigo-50 border-indigo-200 shadow-indigo-100'
            }`}>
              <div className="flex items-start gap-2.5">
                <div className="p-1.5 rounded-xl bg-indigo-600/20 text-indigo-500 border border-indigo-500/30 shrink-0">
                ...
                <div>
                  ...
                  <p className={`text-xs mt-0.5 font-medium ${isDarkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                    {reasoning}
                  </p>
                </div>
              </div>
              ...
              <button
                onClick={handleResetFilters}
                className={`w-full sm:w-auto px-3.5 py-1.5 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors border shrink-0 ${
                  isDarkMode
          ...
          {/* Catalog Controls Header (Tightened Gap to Cards) */}
          <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-2.5 transition-colors ${
            isDarkMode ? 'border-slate-800/80' : 'border-slate-200'
          }`}>
            <div className="flex items-center gap-2.5">
              <ShoppingBag className="w-5 h-5 text-indigo-500 shrink-0" />
              <h2 className={`text-xl sm:text-2xl font-extrabold tracking-tight transition-colors ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                {isFiltered ? 'AI Filtered Products' : 'All Products'}
              </h2>
              <span className={`text-[11px] px-2 py-0.5 rounded-full border font-mono font-normal shrink-0 transition-colors ${
                isDarkMode
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-0.5 sm:pb-0 scrollbar-none">
              {isFiltered && (
                <button
                  onClick={handleResetFilters}
                  className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-rose-950/50 hover:bg-rose-900/60 text-rose-300 border border-rose-800/60 flex items-center gap-1 transition-colors shrink-0"
                >
